...
function getUsersFromDatabase(array) {
  ...
  return Array.from(array, (e, i) => {
   return {id: e['id'], firstName: e['name'].toUpperCase(), years: e['age']}
  })
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    let newArr = []
    let e1 = arr.indexOf(element)
    let e2 = arr.lastIndexOf(element)
    newArr.push(e1)
    if (e1 != e2) newArr.push(e2)
    return newArr
  } else {
    return []
  }
...
if (target in arr && start in arr && end in arr) {
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (Array.isArray(arr) && start in arr && end in arr) {
